      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <Card className="relative overflow-hidden">
          ...
          <CardTitle className="max-w-xl text-4xl leading-tight md:text-5xl">
            Sign in, pull your recent photos, and review their existing metadata.
          </CardTitle>
          <CardDescription className="mt-4 max-w-2xl text-base leading-7">
            Your session is already active. Retrieve your latest photos and review the descriptions and tags that already exist on each image.
          </CardDescription>
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <Badge className="bg-white/90">Authenticated</Badge>
            <Badge className="bg-transparent text-muted">Unsplash session active</Badge>
          </div>
          {authBanner ? (
            <p className="mt-5 text-sm font-medium text-foreground">{authBanner}</p>
          ) : null}
        </Card>
              ...
              <div className="space-y-4 p-5">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-foreground">
                    {photo.description?.trim() || "No description on this photo yet."}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {tags.length > 0 ? (
                    tags.map((tag) => <Badge key={`${photo.id}-${tag}`}>{tag}</Badge>)
                  ) : (
